    ...
    override fun onCommand(sender: CommandSender, cmd1: Command, label: String, args: Array<out String>): Boolean {
        ...
        Thread {
            try {
                ...
                sender.sendMessage("§6Set weapon ability to §a${ability?.name ?: "None"}")
                ...
                val sword:SkyblockSword
                if(ability!=null){
                    sword = SkyblockSword(ItemData(itemName,rarity,mat,reforgeable), SwordStats(damage, strength, critChance, critDamage, intel), ability)
                }else{
                    sword = SkyblockSword(ItemData(itemName,rarity,mat,reforgeable), SwordStats(damage, strength, critChance, critDamage, intel))
                }
                sender.sendMessage("You have received your "+itemName)
                sender.inventory.addItem(sword.createItem())
